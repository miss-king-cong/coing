...
[task_local]
#公众号万达酒店及度假村积分
00 00 7,9,11 * * * jd_speed_sign.js, tag=公众号万达酒店及度假村积分, img-url=https://raw.githubusercontent.com/Orz-3/task/master/jd.png, enabled=true

================Loon==============
[Script]
cron "00 00 7,9,11 * * *" script-path=jd_speed_sign.js,tag=公众号万达酒店及度假村积分

===============Surge=================
公众号万达酒店及度假村积分 = type=cron,cronexp="00 00 7,9,11 * * *",wake-system=1,timeout=33600,script-path=jd_speed_sign.js

============小火箭=========
公众号万达酒店及度假村积分 = type=cron,script-path=jd_speed_sign.js, cronexpr="00 00 7,9,11 * * *", timeout=33600, enable=true
*/

const $ = new Env('公众号万达酒店及度假村积分');
var appUrlArr = [];
var Cookie2 = '';
var WdjdUrl = "";
...
async function checkEnv() {
    if ($.isNode()) {
        WdjdUrl = process.env.WdjdUrl;
    } else {
        WdjdUrl = $.getdata('WdjdUrl');
    }
    if (!WdjdUrl) {
        let str1 = WdjdUrl ? "" : "WdjdUrl";
        $.log(`未找到环境变量: ${str1}\n`);
        return false;
    }
    if (WdjdUrl.indexOf('#') != -1) {
        appUrlArrs = WdjdUrl.split('#');
        $.log(`您选择的是用"#"隔开WdjdUrl\n`);
    } else if (WdjdUrl.indexOf('\n') != -1) {
        appUrlArrs = WdjdUrl.split('\n');
        $.log(`您选择的是用"\\n"隔开WdjdUrl\n`);
    } else if (WdjdUrl.indexOf('@') != -1) {
        appUrlArrs = WdjdUrl.split('@');
        $.log(`您选择的是用"@"隔开WdjdUrl\n`);
    } else {
        appUrlArrs = [WdjdUrl];
        ...
        $.get(url, async (err, resp, data) => {
            try {
                if (err) {
                    ...
                    let data2 = JSON.parse(data);
                    if (data2.success) {
                        var type = data2.data.lottery.type;
                        $.log(`抽奖结果 ` + type);
                    } else {
                        $.log(`抽奖结果` + data2.desc);
                    }
