...
[task_local]
#越秀地产越秀会签到
00 00 8,10 * * * jd_speed_sign.js, tag=越秀地产越秀会签到, img-url=https://raw.githubusercontent.com/Orz-3/task/master/jd.png, enabled=true

================Loon==============
[Script]
cron "00 00 8,10 * * *" script-path=jd_speed_sign.js,tag=越秀地产越秀会签到

===============Surge=================
越秀地产越秀会签到 = type=cron,cronexp="00 00 8,10 * * *",wake-system=1,timeout=33600,script-path=jd_speed_sign.js

============小火箭=========
越秀地产越秀会签到 = type=cron,script-path=jd_speed_sign.js, cronexpr="00 00 8,10 * * *", timeout=33600, enable=true
*/

const $ = new Env('越秀地产越秀会签到');
var appUrlArr = [];
var Cookie2 = '';
var YxdcyxhUrl = "";
var jwt2 = "";
...
async function checkEnv() {
    if ($.isNode()) {
        YxdcyxhUrl = process.env.YxdcyxhUrl;
    } else {
        YxdcyxhUrl = $.getdata('YxdcyxhUrl');
    }
    if (!YxdcyxhUrl) {
        let str1 = YxdcyxhUrl ? "" : "YxdcyxhUrl";
        $.log(`未找到环境变量: ${str1}\n`);
        return false;
    }
    if (YxdcyxhUrl.indexOf('#') != -1) {
        appUrlArrs = YxdcyxhUrl.split('#');
        $.log(`您选择的是用"#"隔开YxdcyxhUrl\n`);
    } else if (YxdcyxhUrl.indexOf('\n') != -1) {
        appUrlArrs = YxdcyxhUrl.split('\n');
        $.log(`您选择的是用"\\n"隔开YxdcyxhUrl\n`);
    } else if (YxdcyxhUrl.indexOf('@') != -1) {
        appUrlArrs = YxdcyxhUrl.split('@');
        $.log(`您选择的是用"@"隔开YxdcyxhUrl\n`);
    } else {
        appUrlArrs = [YxdcyxhUrl];
    }
