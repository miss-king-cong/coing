...
[task_local]
#公众号明宇酒店商城积分
00 00 8 * * * jd_speed_sign.js, tag=公众号明宇酒店商城积分, img-url=https://raw.githubusercontent.com/Orz-3/task/master/jd.png, enabled=true

================Loon==============
[Script]
cron "00 00 8 * * *" script-path=jd_speed_sign.js,tag=公众号明宇酒店商城积分

===============Surge=================
公众号明宇酒店商城积分 = type=cron,cronexp="00 00 8 * * *",wake-system=1,timeout=33600,script-path=jd_speed_sign.js

============小火箭=========
公众号明宇酒店商城积分 = type=cron,script-path=jd_speed_sign.js, cronexpr="00 00 8 * * *", timeout=33600, enable=true
*/
...
async function checkEnv() {
    if ($.isNode()) {
        MyjdUrl = process.env.MyjdUrl;
    } else {
        MyjdUrl = $.getdata('MyjdUrl');
    }
    if (!MyjdUrl) {
        let str1 = MyjdUrl ? "" : "MyjdUrl";
        $.log(`未找到环境变量: ${str1}\n`);
        return false;
    }
    if (MyjdUrl.indexOf('#') != -1) {
        appUrlArrs = MyjdUrl.split('#');
        $.log(`您选择的是用"#"隔开MyjdUrl\n`);
    } else if (MyjdUrl.indexOf('\n') != -1) {
        appUrlArrs = MyjdUrl.split('\n');
        $.log(`您选择的是用"\\n"隔开MyjdUrl\n`);
    } else if (MyjdUrl.indexOf('@') != -1) {
        appUrlArrs = MyjdUrl.split('@');
        $.log(`您选择的是用"@"隔开MyjdUrl\n`);
    } else {
        appUrlArrs = [MyjdUrl];
