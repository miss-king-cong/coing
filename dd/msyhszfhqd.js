...
async function checkEnv() {
    if ($.isNode()) {
        Msyhszfh_mcpCode = process.env.Msyhszfh_mcpCode;
    } else {
        Msyhszfh_mcpCode = $.getdata('Msyhszfh_mcpCode');
    }
    if (!Msyhszfh_mcpCode) {
        let str1 = Msyhszfh_mcpCode ? "" : "Msyhszfh_mcpCode";
        $.log(`未找到环境变量: ${str1}\n`);
        return false;
    }
    if (Msyhszfh_mcpCode.indexOf('#') != -1) {
        appUrlArrs = Msyhszfh_mcpCode.split('#');
        $.log(`您选择的是用"#"隔开Msyhszfh_mcpCode\n`);
