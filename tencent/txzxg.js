...
https://wzq.tenpay.com/cgi-bin/activity_task_daily.fcgi? url script-request-header https://raw.githubusercontent.com/miss-king-cong/coing/main/tencent/txzxg.js
[MITM]
...
let bullTaskArray = {
    ...
    "open_blindbox": {
        "taskName": "开盲盒",
        "action": "open_blindbox",
        "actid": 1105
    },
    "query_blindbox": {
        "taskName": "查询皮肤数量",
        "action": "query_blindbox",
        "actid": 1105
    },
    "sell_skin": {
        "taskName": "卖皮肤",
...
async function initTaskList() {
    ...
    for (let i = 0; i < appActidArray["newbie"].length; i++) {
        taskItem = {
            "taskName": "APP新手任务",
            "activity": "task_continue",
            "type": "app_new_user",
            "actid": appActidArray["newbie"][i]
        }
    ...
    for (let i = 0; i < wxActidArray["newbie"].length; i++) {
        taskItem = {
            "taskName": "微信新手任务",
            "activity": "task_continue",
            "type": "wzq_welfare_growth",
            "actid": wxActidArray["newbie"][i]
        }
                        ...
                        if (result.retcode == 0) {
                            ...
                            if (isGuessTime) {
                                ...
                                if (result.recommend && Array.isArray(result.recommend)) {
                                    ...
                                    stockList = recList.sort(function(a, b) {
                                        return Math.abs(b["zdf"]) - Math.abs(a["zdf"])
                                    });
                                    guessStockFlag = 1
